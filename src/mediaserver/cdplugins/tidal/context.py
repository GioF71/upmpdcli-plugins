...
class ContextException(Exception):
    """Raised when an error occurs in the Context module."""
...
class Context:
    ...
    def __not_null(self, key: ContextKey):
        if not key:
            raise ContextException("Provided key is None")

    def __raise_missing_key(self, key_str: str):
        raise ContextException(f"Missing key [{key_str}]")

    def __not_in(self, key: ContextKey):
        self.__not_null(key)
        if key in self._dict:
            raise ContextException(f"Duplicate key [{key.name}]")
    ...
    def increment(self, key: ContextKey):
        ...
        if not isinstance(v, int):
            raise ContextException(f"Item at key [{key.name}] is not a int")
    ...
    def get(self, key: ContextKey, allow_empty: bool = True):
        ...
        return self._dict.get(key.name, key.default_value)
    ...
    def dict_add(
        ...
        if entry_key in select_dict and not allow_update:
            raise ContextException(f"Duplicate key [{entry_key}]")
    ...
    def dict_remove(
        ...
        if entry_key in select_dict:
            ...
            if not allow_missing:
                raise ContextException(f"Key [{entry_key}] not found")
...
def context_get(context: Context, key: ContextKey, allow_empty: bool = True, allow_null_context: bool = True):
    if not context:
        if not allow_null_context:
            raise ContextException("Context is None")
        return None
    return context.get(key=key, allow_empty=allow_empty)