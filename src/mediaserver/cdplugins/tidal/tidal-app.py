...
def set_stream_information_for_mix_or_playlist_entry(
    ...
    if not bit_depth or not sample_rate or not audio_quality:
        ...
        if not bit_depth: bit_depth = 16
        if not sample_rate: sample_rate = 44100
        if not audio_quality: audio_quality = config.get_fallback_quality_when_missing()
...
def song_listening_queue_art_retriever() -> str:
    select_track_id: str = __get_random_track_id_from_listen_queue()
    select_track: TidalTrack
    select_track, _ = (tidal_util.try_get_track(get_session(), select_track_id)
                       if select_track_id else None, None)
    return tidal_util.get_image_url(select_track.album) if select_track else None
...
def artist_listening_queue_art_retriever() -> str:
    select_artist_id : str = __get_random_artist_id_from_listen_queue()
    select_artist : TidalArtist = (tidal_util.try_get_artist(get_session(), select_artist_id)
...
def handler_tag_bookmarks(
    ...
    for current_tuple in tuple_array:
        ...
        entry : dict[str, any] = upmplgutils.direntry(id,
            objid,
            current_tuple[1])
        image_url : str = current_tuple[2]() if current_tuple[2] else None
...
def page_to_entries(
        ...
        pagelink_identifier: tidal_util.PageLinkIdentifier = None) -> list:
    ...
    if next_item:
        ...
        if next_button_element_type:
            ...
            if ElementType.TAG == next_button_element_type:
                ...
                if ElementType.PAGELINK == next_button_element_type:
                    if (pagelink_identifier
                            and pagelink_identifier.api_path
                            and pagelink_identifier.category_title):
                        next_entry = create_next_button(
                            objid=objid,
...
def handler_element_album_container(
    ...
    if not album:
        ...
        entries.append(create_missing_album_entry(objid, album_id))
        return entries
    ...
    _add_album_tracks_entry(
        objid=objid,
        album=album,
        entries=entries)
    # add Artists
    _add_album_artists_entries(
        objid=objid,
        album=album,
        entries=entries)
    # add favorite action
...
def _add_album_artists_entries(
        objid,
        album: TidalAlbum,
        entries: list):
    ...
    for tile_idx in range(0, tile_count):
        ...
        if isinstance(mix_or_playlist, TidalPlaylist):
            ...
            if tile_idx == 0:
                ...
                track_list : list[TidalTrack] = playlist.tracks(offset = 0, limit = max_items)
...
def handler_element_track_container(objid, item_identifier : ItemIdentifier, entries : list) -> list:
    ...
    if track:
        ...
        entries.append(create_missing_track_entry(objid=objid, track_id=track_id))
    return entries
...
def handler_element_bookmark_albums(objid, item_identifier : ItemIdentifier, entries : list) -> list:
    ...
    for obj_id in obj_list:
        ...
        try:
            ...
            if tidal_obj:
                success_count += 1
                entries.append(album_to_album_container(objid, tidal_obj))
...
def get_tidal_album_loader() -> Callable[[str], TidalAlbum]:
    ...
    if len(path_list) == 1 and _g_myprefix == last_path_item:
        ...
        if ElementType.TAG.getName() == thing_name:
            ...
            if tag_handler:
                ...
                msgproc.log(f"no tag handler for [{thing_value}]")
            ...
            if elem_handler:
                ...
                # msgproc.log(f"Skip cache for [{thing_name}]: [{no_elem_cache}]")
                return _returnentries(entries, no_cache=no_elem_cache)
            else:
                msgproc.log(f"no element handler for [{thing_name}]")
                return _returnentries(entries)
