...
class Session(object):
    ...
    def get_media_info(self, trackid):
        info = self.api.track_getFileUrl(
            intent="stream", track_id=trackid, format_id=self.format_id
        )
    ...
    def get_featured_albums(self, genre_id="None", type="new-releases"):
        ...
        if genre_id != "None":
            data = self.api.album_getFeatured(
                type=type, genre_ids=genre_id + ":", limit=general_slice
            )
        else:
...
def _parse_album(json_obj, artist=None, artists=None):
    ...
    name = json_obj["title"]
    if (
        show_album_rate_and_bits
        and "maximum_sampling_rate" in json_obj
        and "maximum_bit_depth" in json_obj
    ):
        name += (
            " ("
            + str(int(json_obj["maximum_sampling_rate"]))
            + "/"
...
class Favorites(object):
    ...
    def artists(self):
        return self.session.loopget(
            0,
            self.session.api.favorite_getUserFavorites,
    ...
    def playlists(self):
        return self.session.loopget(
            0,
            self.session.api.playlist_getUserPlaylists,
